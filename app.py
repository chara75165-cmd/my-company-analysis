...
with tab1:
    ...
    if st.session_state.current_analysis:
        ...
        with col_g:
            cats = ['収益性', '安全性', '成長性']
            fig = go.Figure(data=go.Scatterpolar(r=scores + [scores[0]], theta=cats + [cats[0]], fill='toself'))
        ...
        with col_v:
            ...
            if ind_avg:
                am, asf, at = ind_avg
                st.metric("営業利益率", f"{margin:.1f}%", f"{margin - am:.1f}%")
                st.metric("自己資本比率", f"{safety:.1f}%", f"{safety - asf:.1f}%")
                st.metric("成長トレンド", f"{trend:.1f}%", f"{trend - at:.1f}%")
        ...
        elif safety > 70: d_l, d_c = "🏯 超安定型", "抜群の財務基盤。長く働ける環境です。"
        ...
        if st.button("⭐ お気に入りに追加"):
            if not any(f['企業名'] == curr['name'] for f in st.session_state.fav_list):
                st.session_state.fav_list.append({
                    '企業名': curr['name'], '利益率': f"{margin:.1f}%", '安全性': f"{safety:.1f}%", '成長性': f"{trend:.1f}%"
                })
                st.toast("追加しました！")
    ...
    if st.button("⚔️ 比較を開始", key="c_btn"):
        ...
        if res1 and res2:
            ...
            with col_cg:
                fig_c = go.Figure()
                ct = ['収益性', '安全性', '成長性']
                fig_c.add_trace(go.Scatterpolar(r=s1+[s1[0]], theta=ct+[ct[0]], fill='toself', name=c1_n))
                fig_c.add_trace(go.Scatterpolar(r=s2+[s2[0]], theta=ct+[ct[0]], fill='toself', name=c2_n))
